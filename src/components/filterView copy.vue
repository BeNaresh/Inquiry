          ...
          <v-tabs-items v-model="tab">
            <v-tab-item v-for="item in tabList" :key="item">
              ...
              <v-card-text v-if="item == 'filtering'" class="pa-0 ma-0">
                ...
                <list-box
                  :headerVal="localfilterList"
                  :defaultHeader="defaultShopList"
                  @removeList="removeSortChan($event)"
                  @addList="addSortchan($event)"
                ></list-box>
                <list-box
                  :headerVal="shopList"
                  :defaultHeader="localfilterselectedList"
                  @removeList="removeShopList($event)"
                  @addList="addShopList($event)"
                ></list-box>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
import { mdiFilter, mdiInformationOutline, mdiCalendarMonth } from "@mdi/js";
export default {
  data() {
    return {
      tab: 0,
  ...
  props: ["headerVal", "defaultHeader", "filterList", "filterSelect"],
  components: {
    "list-box": ListBox,
